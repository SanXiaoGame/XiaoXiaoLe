...
public class Equipments : MonoBehaviour {
    ...
    [HideInInspector]
    internal int ID { get; set; }//装备ID
    [HideInInspector]
    internal string type;//装备的角色职业类型
    bool isCome = false;
    ...
    private void Awake()
    {
        ...
        target = GameObject.FindGameObjectWithTag("Bag").gameObject;
    ...
    void EquipmentAddAndRemove() {
        ...
        if (topBtn.ID != 0 && SQLiteManager.Instance.playerDataSource[topBtn.ID].player_Class == type)
        {
            ...
            if (!isCome)
            {
                target.GetComponent<CreateEquipts>().itemLists.Remove(this.gameObject);
                for (int i = 0; i < equiptHole.Length; i++)
                {
                    if (equiptHole[i].childCount == 0)
                    {
                        this.transform.parent = equiptHole[i];
                        this.transform.localPosition = Vector3.zero;
                        isCome = true;

                        EquipmentData equipmentData = SQLiteManager.Instance.equipmentDataSource[ID];
                        HeroData hero = SQLiteManager.Instance.team[topBtn.ID];
            ...
            {
                target.GetComponent<CreateEquipts>().itemLists.Add(this.gameObject);
                for (int i = 0; i < bagLists.Length; i++)
                {
                    if (bagLists[i].childCount == 0)
                    {
                        this.transform.parent = bagLists[i];
                        this.transform.localPosition = Vector3.zero;
                        EquipmentData equipmentData = SQLiteManager.Instance.equipmentDataSource[ID];
