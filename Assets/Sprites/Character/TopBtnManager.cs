...
public class TopBtnManager : MonoBehaviour {
    ...
    internal int ID;//战斗位置选择的角色的ID
    
    internal GameObject characterPoprety;//角色的信息面板
    
    internal GameObject bag;//角色的背包
    ...
    private void Awake()
    {
        ...
        this.transform.Find("Person").GetComponent<Toggle>().onValueChanged.AddListener((isOn)=> {
            ...
            if (skillHole.GetComponent<SkillMessageShow>() == null)
            {
                skillHole.AddComponent<SkillMessageShow>();
            }
            if (ID != 0)
            {
                ...
                skillHole.GetComponent<SkillMessageShow>().RefreshSkill(ID);
            }
